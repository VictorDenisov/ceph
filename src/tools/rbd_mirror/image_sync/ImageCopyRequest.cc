...
int ImageCopyRequest<I>::compute_snap_map() {
  ...
  {
    RWLock::RLocker snap_locker(m_remote_image_ctx->snap_lock);
    snap_id_end = m_remote_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace(), m_sync_point->snap_name); //TODO
    ...
    if (!m_sync_point->from_snap_name.empty()) {
      snap_id_start = m_remote_image_ctx->get_snap_id(cls::rbd::UserSnapshotNamespace(), // TODO

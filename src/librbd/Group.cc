...
static int group_snap_remove_by_record(librados::IoCtx& group_ioctx,
  ...
  image_snap_name = calc_ind_image_snap_name(group_ioctx.get_id(),
					     group_id,
					     group_snap.id);
  for (int i = 0; i < snap_count; ++i) {
    ...
    ictx->operations->snap_remove(image_snap_name.c_str(), on_finishes[i]);
  }
...
remove_image_snaps:
  ...
  for (int i = 0; i < image_count; ++i) {
    ...
    on_finishes[i] = new C_SaferCond;
    ictx->operations->snap_remove(ind_snap_name.c_str(), on_finishes[i]);
  }

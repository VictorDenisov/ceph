...
public:
  ...
  void create_snapshot(std::string snap_name="snap1", bool protect=false) {
    ...
    EXPECT_EQ(0, ictx->operations->snap_create(snap_name.c_str(), cls::rbd::UserSnapshotNamespace()));
  ...
  std::string create_clone() {
    ...
    EXPECT_EQ(0, ictx->operations->snap_create("snap1", cls::rbd::UserSnapshotNamespace()));
